...
from misc import rgb2gray, geometry_3d as geom3d
...
class VisionForwardModel:
    ...
    def __init__(self, render_size=DEFAULT_RENDER_SIZE, camera_pos=DEFAULT_CAMERA_POS,
        ...
        self.cube_mapper.SetInputData(self.cube_output)
    ...
    def render(self, shape, rgb=False):
        ...
        if rgb:
            img_arr = np.zeros((viewpoint_count, 3, h, w), dtype=np.uint8)
        else:
            img_arr = np.zeros((viewpoint_count, h, w))
...
if __name__ == '__main__':
    forward_model = VisionForwardModel(render_size=(200, 300), offscreen_rendering=False, custom_lighting=False)

    import shape as hyp

    s = hyp.Shape(forward_model)
    print(s)
    forward_model._view(s)
